...
class Zen_Cortext {
    ...
    private function init_hooks() {
        ...
        // Registers [zen_cortext_author_bio] / [zen_cortext_author_posts_heading]
    ...
    public function init() {
        ...
        // (Removed in 2.39.19) A one-time GTM→custom-code migration formerly
        // lived here and authored a tracking snippet directly in plugin source
        // (WordPress.org review flags any plugin-authored script markup, even
        // when only stored into an option). Tracking code is now strictly
        // admin-pasted via the Tracking tab's Header/Body/Footer custom-code
        // fields — nothing is auto-generated by the plugin. Existing installs
        // keep whatever they previously stored in those options.
    ...
    public static function create_tables() {
        ...
        $chats_sql = "CREATE TABLE {$chats_table} (
            ...
            amv VARCHAR(128) NOT NULL DEFAULT '',
        ...
        // MySQL/MariaDB setups (see the ads-table note below), so add the `amv`
        // column (the a-metrics first-party visitor id, captured from the
        // `_amv_js` cookie for cross-tool journey stitching) explicitly and
        // idempotently on both the chats and sessions tables.
        $chats_cols = $wpdb->get_col("DESC {$chats_table}", 0);
        if (!in_array('amv', $chats_cols, true)) {
            $wpdb->query("ALTER TABLE {$chats_table} ADD COLUMN amv VARCHAR(128) NOT NULL DEFAULT '' AFTER fbp");
        }
        $chats_amv_idx = $wpdb->get_results($wpdb->prepare("SHOW INDEX FROM {$chats_table} WHERE Key_name = %s", 'amv'));
        if (empty($chats_amv_idx)) {
            $wpdb->query("ALTER TABLE {$chats_table} ADD KEY amv (amv)");
        }
        $sessions_cols = $wpdb->get_col("DESC {$sessions_table}", 0);
        if (!in_array('amv', $sessions_cols, true)) {
            $wpdb->query("ALTER TABLE {$sessions_table} ADD COLUMN amv VARCHAR(128) NOT NULL DEFAULT '' AFTER fbp");
        }

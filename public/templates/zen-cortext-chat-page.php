...
// $zen_cortext_intro is consumed by public/views/chat.php (the intro card markup).
...
$zen_cortext_team_user_ids = array_values(array_map('intval', (array) get_option('zen_cortext_invitable_users', array())));

$zen_cortext_team_members = array();
foreach ($zen_cortext_team_user_ids as $zen_cortext_uid) {
    $zen_cortext_u = get_userdata($zen_cortext_uid);
    if (!$zen_cortext_u) continue;

    $zen_cortext_avatar_att = (int) get_user_meta($zen_cortext_uid, 'zen_user_avatar', true);
    $zen_cortext_avatar_url = $zen_cortext_avatar_att ? wp_get_attachment_image_url($zen_cortext_avatar_att, 'thumbnail') : get_avatar_url($zen_cortext_uid, array('size' => 80));

    $zen_cortext_email    = get_the_author_meta('author_email', $zen_cortext_uid);
    if (empty($zen_cortext_email)) $zen_cortext_email = $zen_cortext_u->user_email;
    $zen_cortext_whatsapp = get_the_author_meta('author_whatsapp', $zen_cortext_uid);
    $zen_cortext_linkedin = get_the_author_meta('author_linkedin', $zen_cortext_uid);
    $zen_cortext_wa_num   = $zen_cortext_whatsapp ? preg_replace('/[^0-9]/', '', $zen_cortext_whatsapp) : '';
    $zen_cortext_role     = function_exists('zen_get_user_role')
        ? zen_get_user_role($zen_cortext_uid)
        : trim((string) get_user_meta($zen_cortext_uid, 'zen_user_role', true));

    $zen_cortext_team_members[] = array(
        'name'       => $zen_cortext_u->display_name,
        'role'       => $zen_cortext_role,
        'avatar_url' => $zen_cortext_avatar_url,
        'profile'    => get_author_posts_url($zen_cortext_uid),
        'email'      => $zen_cortext_email,
        'wa_num'     => $zen_cortext_wa_num,
        'linkedin'   => $zen_cortext_linkedin,
    );
}
...
$zen_cortext_saved_links = (array) get_option('zen_cortext_quick_links', Zen_Cortext_Defaults::default_quick_links());
$zen_cortext_quick_links = array();
foreach ($zen_cortext_saved_links as $zen_cortext_row) {
    if (!is_array($zen_cortext_row)) continue;
    $zen_cortext_url   = isset($zen_cortext_row['url'])   ? (string) $zen_cortext_row['url']   : '';
    $zen_cortext_label = isset($zen_cortext_row['label']) ? (string) $zen_cortext_row['label'] : '';
    if ($zen_cortext_url === '' && $zen_cortext_label === '') continue;
    $zen_cortext_quick_links[] = array(
        'href'   => $zen_cortext_url,
        'target' => isset($zen_cortext_row['target']) ? (string) $zen_cortext_row['target'] : '',
        'icon'   => isset($zen_cortext_row['icon'])   ? (string) $zen_cortext_row['icon']   : '',
        'label'  => $zen_cortext_label,
        'prefix' => isset($zen_cortext_row['prefix']) ? (string) $zen_cortext_row['prefix'] : '',
    );
}

ob_start();
foreach ($zen_cortext_team_members as $zen_cortext_member) { ?>
    <div class="zcp-team-card">
      <div class="zcp-team-top">
        <a class="zcp-team-left" href="<?php echo esc_url($zen_cortext_member['profile']); ?>" target="_blank" rel="noopener noreferrer" title="<?php echo esc_attr($zen_cortext_member['name']); ?>">
            <img class="zcp-team-avatar" src="<?php echo esc_url($zen_cortext_member['avatar_url']); ?>" alt="<?php echo esc_attr($zen_cortext_member['name']); ?>" width="52" height="52" loading="eager" />
            <div class="zcp-team-name"><?php echo esc_html($zen_cortext_member['name']); ?></div>
        </a>
        <div class="zcp-team-buttons">
            <?php if (!empty($zen_cortext_member['email'])) : ?>
                <a href="mailto:<?php echo esc_attr($zen_cortext_member['email']); ?>" class="zcp-team-btn zcp-team-btn-email" data-email="<?php echo esc_attr($zen_cortext_member['email']); ?>" title="<?php esc_attr_e('Copy email', 'zen-cortext'); ?>">
            ...
            <?php endif; ?>
            <?php if (!empty($zen_cortext_member['wa_num'])) : ?>
                <a href="https://wa.me/<?php echo esc_attr($zen_cortext_member['wa_num']); ?>" class="zcp-team-btn zcp-team-btn-wa" target="_blank" rel="noopener noreferrer" title="<?php esc_attr_e('Chat on WhatsApp', 'zen-cortext'); ?>">
            ...
            <?php endif; ?>
            <?php if (!empty($zen_cortext_member['linkedin'])) : ?>
                <a href="<?php echo esc_url($zen_cortext_member['linkedin']); ?>" class="zcp-team-btn zcp-team-btn-li" target="_blank" rel="noopener noreferrer" title="<?php esc_attr_e('LinkedIn', 'zen-cortext'); ?>">
      ...
      <?php if (!empty($zen_cortext_member['role'])) : ?>
        <div class="zcp-team-role"><?php echo esc_html($zen_cortext_member['role']); ?></div>
      <?php endif; ?>
    </div>
<?php }

foreach ($zen_cortext_quick_links as $zen_cortext_ql) {
    ...
    $zen_cortext_target_attr = ' target="_blank" rel="noopener"';
    $zen_cortext_ext_icon    = ' <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="opacity:.5;vertical-align:middle;margin-left:2px"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>';
    $zen_cortext_prefix      = !empty($zen_cortext_ql['prefix']) ? '<span class="zcp-rail-btn-prefix">' . esc_html($zen_cortext_ql['prefix']) . '</span> ' : '';
    ?>
    <a class="zcp-rail-btn" href="<?php echo esc_url($zen_cortext_ql['href']); ?>"<?php echo $zen_cortext_target_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- hardcoded attribute fragment, safe. ?>>
        <span class="zcp-rail-btn-icon" aria-hidden="true"><?php echo esc_html($zen_cortext_ql['icon']); ?></span>
        <span class="zcp-rail-btn-label"><?php
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $zen_cortext_prefix and $zen_cortext_ext_icon are hardcoded markup built above ($zen_cortext_prefix wraps esc_html'd text; $zen_cortext_ext_icon is a static external-link SVG); $zen_cortext_ql['label'] is esc_html'd inline. wp_kses_post would strip the SVG.
            echo $zen_cortext_prefix . esc_html($zen_cortext_ql['label']) . $zen_cortext_ext_icon;
        ?></span>
    </a>
<?php }
$zen_cortext_buttons_html = ob_get_clean();
?><!DOCTYPE html>
...
$zen_cortext_chat_font_family_opt = trim((string) get_option('zen_cortext_font_family', ''));
$zen_cortext_chat_font_family     = $zen_cortext_chat_font_family_opt !== ''
    ? $zen_cortext_chat_font_family_opt
    : Zen_Cortext_Defaults::font_family_standalone_fallback();

$zen_cortext_chat_font_size_opt = (int) get_option('zen_cortext_font_size', 0);
$zen_cortext_chat_font_size_px  = $zen_cortext_chat_font_size_opt > 0
    ? $zen_cortext_chat_font_size_opt
...
$zen_cortext_body_css     = 'body.zcp-body{font-family:' . Zen_Cortext_Shortcode::sanitize_font_family($zen_cortext_chat_font_family)
                 . ';font-size:' . (int) $zen_cortext_chat_font_size_px . 'px;}';
$zen_cortext_override_css = Zen_Cortext_Shortcode::build_color_override_css();
...
wp_add_inline_style('zen-cortext-chat-page', $zen_cortext_body_css . $zen_cortext_override_css);
...
$zen_cortext_chat_html = ob_get_clean();

$zen_cortext_chat_page_body = Zen_Cortext_Template_Renderer::render('chat-page-body.tpl.html', array(
    'rail_buttons_html'    => $zen_cortext_buttons_html, // already sanitized at construction time
    'chat_html'            => $zen_cortext_chat_html,
...
// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $zen_cortext_chat_page_body is full page HTML (SVG icons + Alpine.js directives + the chat shell) produced by Zen_Cortext_Template_Renderer, which escapes EVERY dynamic placeholder itself (esc_html for {{ key }}, esc_url for {{ url: }}, esc_attr for {{ attr: }}; only pre-built {{ raw: }} HTML is passed through). The two raw inputs — $zen_cortext_buttons_html and $zen_cortext_chat_html — are assembled above/in chat.php with per-field esc_html()/esc_url()/esc_attr(). wp_kses_post() can't be used here: it would strip the SVG elements and Alpine attributes and break the page.
echo $zen_cortext_chat_page_body;
?>
